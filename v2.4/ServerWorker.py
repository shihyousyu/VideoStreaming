...
class ServerWorker:
	...
	def processRtspRequest(self, data):
		...
		# Process LIST request (before any session exists)
		if requestType == self.LIST:
			print("processing LIST\n")
			files = self.listMovies()
	...
	def listMovies(self):
		"""Return the .Mjpeg file names in the server's working directory."""
		names = []
		for f in os.listdir('.'):
			if f.lower().endswith('.mjpeg'):
				names.append(f)
		return sorted(names)
